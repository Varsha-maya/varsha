...
//   ]
  ...
  const[res,setRes]=useState(0)
    ...
    const handleonSubmit = (e) => {
        ...
        fetch("http://localhost:4000/user",reqOptions).then(res=>res.json()).then(data=>{
            console.log("saved");
            setRes(data.id);
        })
    ...
    const validate = (values) => {
      ...
      useEffect(()=>{
        console.log("Executed after render method==============");
        fetch("http://localhost:4000/departments").then(res=>res.json()).then(data=>{
            console.log(data);
            setDepartments(data);
        }).catch(e=>{
            console.error("ERR in dept....");
        }).finally(()=>{
            console.log("Good");
        })
      },[])
      useEffect(()=>{
        console.log("Executed after render method==============");
        fetch("http://localhost:4000/batches").then(res=>res.json()).then(data=>{
            console.log(data);
            setBatches(data);
        }).catch(e=>{
            console.error("ERR in bat....");
        }).finally(()=>{
            console.log("Good");
        })
      },[])
